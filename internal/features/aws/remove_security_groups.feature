...
Feature: Service apply
  ...
  Scenario: Remove security groups
    ...
    And I apply the definition "aws6.yml"
    ...
    And message "firewall.update.aws-fake" number "0" should contain "null" as json field "rules.egress.1.ip"
    And message "firewall.update.aws-fake" number "0" should contain "10.1.1.11/32" as json field "rules.ingress.0.ip"
    And message "firewall.update.aws-fake" number "0" should contain "80" as json field "rules.ingress.0.from_port"
    And message "firewall.update.aws-fake" number "0" should contain "80" as json field "rules.ingress.0.to_port"
    And message "firewall.update.aws-fake" number "0" should contain "-1" as json field "rules.ingress.0.protocol"
    And message "firewall.update.aws-fake" number "0" should contain "null" as json field "rules.ingress.1.ip"
