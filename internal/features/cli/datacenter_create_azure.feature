...
Feature: Ernest project create
  ...
  Scenario: Logged user azure project creation
    ...
    When I run ernest with "project create azure --subscription_id subid --client_id cliid --region westus --tenant_id tenid --client_secret clisec tmp_project"
  ...
  Scenario: Adding an already existing azure project
    ...
    And I'm logged in as "usr" / "secret123"
    When I run ernest with "project create azure --subscription_id subid --client_id cliid --region westus --tenant_id tenid --client_secret clisec tmp_project"
    And I run ernest with "project create azure --subscription_id subid --client_id cliid --region westus --tenant_id tenid --client_secret clisec tmp_project"
